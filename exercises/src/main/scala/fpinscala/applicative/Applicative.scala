  ...
  def compose[G[_]](
    ...
    new Applicative[({ type f[x] = F[G[x]] })#f] {
      ...
      override def map2[A, B, C](fa: F[G[A]], fb: F[G[B]])(
          f: (A, B) => C): F[G[C]] = self.map2(fa, fb)(G.map2(_, _)(f))
    }
  ...
  def validationApplicative[E]
    ...
    new Applicative[({ type f[x] = Validation[E, x] })#f] {
      ...
      override def map2[A, B, C](fa: Validation[E, A], fb: Validation[E, B])(
        ...
        case (Failure(ae, aes), Failure(be, bes)) =>
          Failure(ae, (aes :+ be) ++ bes)
...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] {
  ...
  def compose[G[_]](
    ...
    new Traverse[({ type f[x] = F[G[x]] })#f] {
      override def traverse[AG[_], A, B](fa: F[G[A]])(f: A => AG[B])(
          implicit AG: Applicative[AG]): AG[F[G[B]]] = {
...
//  println("res2 = " + res2)
  ...
  val treeTraverse = new Traverse[Tree]() {
    override def traverse[G[_], A, B](fa: Tree[A])(f: A => G[B])(
      ...
      G.map2(
        f(fa.head),
        listTraverse.traverse(fa.tail)(tree => traverse(tree)(f)))((b, list) =>
        Tree(b, list))
  }
}
